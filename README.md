...
where $\mathbf{T} = \left[u^3, u^2, u, 1\right]$, $\mathbf{Q}_i = \left[q_i, q_{i+1}, q_{i+2}, q_{i+3}\right]^T$ is the vector that collects the local control points and $\mathbf{M}$ is a $4 \times 4$ matrix which selects which spline base is used, typically B-Spline or Catmull-Rom spline (CR-Spline).
